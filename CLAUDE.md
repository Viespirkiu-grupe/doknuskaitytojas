...
- `unoserver` runs as a child process inside the same container, spawned by `utils/libreoffice.js` on startup; on timeout it is SIGKILL'd and restarted automatically; `tini` (PID 1) prevents zombie processes
